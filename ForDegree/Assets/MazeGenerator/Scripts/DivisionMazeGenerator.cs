...
public class DivisionMazeGenerator : BasicMazeGenerator
{
    ...
    ///<summary>
    /// Class representing maze area to be divided (minus - no way of knowing if the area is a goal!)
    ///</summary>
    ...
    public override void GenerateMaze()
    {
        ...
        while (rectsToDivide.Count > 0)
        {
            ...
            if (width > 1 && height > 1)
            {
                ...
                {
                    if (Random.Range(0, 100) > 42) // make sure vertical divides better
                    {
                        divideVertical(currentRect);
                    }
                    else
    ...
    ///<summary>
    ///Divides selected area vertically (colums)
    ///</summary>
    ...
    ///<summary>
    ///Divides selected area horiszontally (rows)
    ///</summary>
    private void divideHorizontal(IntRect rect)
    {
		// Take a random row (one of the horizontal ones)
        ...
        GetMazeCell(divRow, space).WallFront = false; // take random in full walls row, and make a pass on top
        if (divRow + 1 < rect.top)
        {
            GetMazeCell(divRow + 1, space).WallBack = false;// make sure the to cell also has a pass on the bottom
        }
